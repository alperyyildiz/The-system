    ...
    def Create_Block(self):
        ...
        while Add:
            ...
            if TYPE == '1':
                ...
                Dict_Block[ entity_key ] = INP_BLOCK
                ...
                input_branch = input()
                Dict_Block[ entity_key ] = self.Branches_Created[ input_branch ]
        ...
        entity_list_str = list(Dict_Block.keys())
        print('entity_list_str     {}'.format(entity_list_str))
        print('BLOCKS   {}'.format(self.Blocks.keys()))
        print('BLOCKS   {}'.format(Dict_Block.keys()))
        ...
        for i, ENTITY in enumerate(entity_list_str):
            ...
            if entity_type_str[i] == 'block':
                ...
                Flat = self.check_flatten_block(Dict_Block [ ENTITY ])
                if  Flat == True:
                    raise Exception('Flatten can only placed in the end of the block')
                else:
                    #IF THIS IS NOT LAST PART
                    if ENTITY != entity_list_str[-1] :
                        Dict_Block[ ENTITY ] = self.push_ch_to_block(Dict_Block [ ENTITY ] , out_channels )
                        #IF DIM IS 1
                        if self.check_dim( Dict_Block[ ENTITY ] ) == 1:
                            out_channels, _ = self.track_dim_and_ch_block( Dict_Block [ ENTITY ], out_channels, dim=(10000) )
                        #IF DIM IS 2
                        elif self.check_dim( Dict_Block[ ENTITY ] ) == 2:
                            out_channels, _ = self.track_dim_and_ch_block( Dict_Block [ ENTITY ], out_channels, dim=(10000,10000) )

            elif entity_type_str[i] == 'branch':
                dim = (10000)
                dim_2d = (10000,10000)
                dim_list_1d = [dim for x in range(len(list(Dict_Block [ ENTITY ].keys())))]
                dim_list_2d = [dim_2d for x in range(len(list(Dict_Block [ ENTITY ].keys())))]
                Flat = self.check_flatten_branch(Dict_Block [ ENTITY ][1])
                #IF BLOCK IS NOT FLATTENED
                if Flat == True:
                    raise Exception('Flatten can only placed in the end of the block')
                else:
                    #IF THIS IS NOT LAST PART
                    if ENTITY != entity_list_str[-1] :
                        Dict_Block[ entity ] = self.push_ch_to_branch( Dict_Block [ ENTITY ] , out_channels)
                        #IF DIM IS 1
                        if self.check_dim( Dict_Block[ ENTITY ] ) == 1:
                            out_channels, _ = self.track_dim_and_ch_branch( Dict_Block [ ENTITY ], out_channels, dim_list_1d )
                        #IF DIM IS 2
                        elif self.check_dim( Dict_Block[ ENTITY ] ) == 2:
                            out_channels, _ = self.track_dim_and_ch_branch( Dict_Block [ ENTITY ], out_channels, dim_list_2d )
...
class NET(MAIN_OBJ):
    ...
    def push_ch_to_layer(self,Layer,out_chanels):
        LAYER = copy.deepcopy(Layer)
        if LAYER[0] in self.PUSH_OUT_CHANNELS_TO:
    ...
    def track_dim_and_ch_block(self, BLOCK, out_channels, dim):
        ...
        for i, entity in enumerate(block_keys):
            if entity_type[i] == 'layer':
                ...
                out_channels, dim = self.track_dim_and_ch_block(BLOCK[ entity ], out_channels, dim, num_of_dim)
